...
- **Agent configuration goes in `.agents/`** — use `.agents/` for instructions, skills, and agent config in any repo. Do not use `.copilot/`, `.claude/`, or other tool-specific directories. The `COPILOT_CUSTOM_INSTRUCTIONS_DIRS` env var includes both `$HOME/.agents` (global) and `.agents` (per-repo), so Copilot CLI picks up instructions from both locations.
