---
name: obsidian-daily
description: "Create or update today's daily note - pulls calendar events, overdue tasks, and conversation context Triggers: todays note, create todays daily, open daily, today daily note. Use proactively: trigger this whenever the conversation produces something worth capturing, without waiting to be asked."
metadata:
  category: vault
---
...
3. Check if today's `YYYY-MM-DD.md` exists in the daily folder, resolved per `.agents/skills/obsidian-core/references/folder-map.md` (wiki-style `wiki/daily/`, Obsidian-style `Daily/`)
...
5. Pull overdue and due-today tasks from kanban boards:
   - Scan the boards folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md`) for items with `@{date}` that match today or are past due
