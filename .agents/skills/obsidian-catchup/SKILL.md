---
name: obsidian-catchup
description: "Review and process everything captured on the go from the Telegram journal bot - voice, text, images, PDFs, links - waiting in the catchup queue. You pull it when you are back at the laptop; nothing is processed autonomously. Triggers: catch up, catchup, what did I dump from telegram, process my captures, go through my telegram dumps, anything new from the phone, process my catchup, review what I captured, what did I capture on the go. Use only when the user explicitly asks for it."
metadata:
  category: vault
---
...
5. Process the items WITH the user (this is a together-review, never an autonomous sweep). For each item - or a sensible batch - open the linked note/entry (or work from the queue summary when there is no link), read the actual captured content, and propose one of:
   - **Integrate** - fold it into the right existing or new note (person / project / concept / decision), following `.agents/skills/obsidian-core/references/ai-first-rules.md`: update existing notes, fill and link, reconcile contradictions - exactly as `/obsidian-save` would. Prefer updating an existing note over creating a near-duplicate.
