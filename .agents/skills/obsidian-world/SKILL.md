---
name: obsidian-world
description: "Load your identity, values, priorities, and current state in one shot - with progressive context levels to avoid burning tokens Triggers: load context, what is going on, where am I, load my world. Use only when the user explicitly asks for it."
metadata:
  category: vault
---
...
2. Load context progressively - start light, go deeper only as needed:
   ...
   **L3 - Deep Context (on demand, ~5-20K tokens)**
   ...
   - Read full source/reference notes (the concept or knowledge folder per `.agents/skills/obsidian-core/references/folder-map.md` - wiki-style `wiki/concepts/` or `raw/`, Obsidian-style `Knowledge/`) if the user asks about a specific topic
