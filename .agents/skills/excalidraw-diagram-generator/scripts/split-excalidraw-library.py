...
def split_library(library_dir: str) -> None:
    ...
    for item in library_items:
        ...
        filename_stem = sanitize_filename(icon_name)
        filename_counts[filename_stem] += 1
        suffix = filename_counts[filename_stem]
        filename = f'{filename_stem}-{suffix}.json' if suffix > 1 else f'{filename_stem}.json'
