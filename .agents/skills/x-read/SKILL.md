---
name: x-read
description: "Deep-read an X (Twitter) post via Grok x_search - verbatim post, thread, TL;DR, claims, reply sentiment, voices to watch Triggers: read this x post, deep read this tweet, analyze this tweet, read this thread. Use only when the user explicitly asks for it."
metadata:
  category: research
---
...
2. Run the script from the skill root (its absolute path was given at session start as **Skill root**; substitute it for `.agents/skills/obsidian-core`):
   ```bash
   uv run --directory ".agents/skills/obsidian-core" -m scripts.research.x_read "<url>"
   ```
