...
description: "Refresh the vault's semantic search index and report coverage before and after Triggers: reindex vault, rebuild semantic index, refresh semantic search, update vault embeddings. Use only when the user explicitly asks for it."
metadata:
  category: meta
---
...
1. Read `_CLAUDE.md` first to find the vault path. Resolve it to an absolute path and substitute it for `VAULT_PATH` below. The skill root was given at session start as **Skill root**; substitute its absolute path for `.agents/skills/obsidian-core`.

2. Measure coverage before changing the index:
   ```bash
   uv run --directory ".agents/skills/obsidian-core" python -c 'import json, sys; from pathlib import Path; sys.path.insert(0, str(Path(sys.argv[1]) / "integrations" / "obsidian-mcp-server")); from vault_ops import index_coverage; print(json.dumps(index_coverage(Path(sys.argv[2]).expanduser().resolve())))' ".agents/skills/obsidian-core" "VAULT_PATH"
   ```
...
3. Tell the user that the build is incremental, then run it from the skill root:
   ```bash
   uv run --directory ".agents/skills/obsidian-core" python scripts/eval/semantic_search.py --path "VAULT_PATH" --build
   ```
