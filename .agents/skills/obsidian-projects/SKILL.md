...
description: "Live project status from git + local docs - infers all context from vault notes, no config required Triggers: projects overview, project status, what am I working on, show projects. Use only when the user explicitly asks for it."
metadata:
  category: vault
---
...
Read `_CLAUDE.md` to find the projects folder name, resolved per `.agents/skills/obsidian-core/references/folder-map.md` (wiki-style `wiki/projects/`, Obsidian-style `Projects/`). If not defined, default to the wiki-style `wiki/projects/`.
