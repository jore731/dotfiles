---
name: create-command
description: "Create a new obsidian-second-brain command via interview - zero markdown editing required Triggers: create command, new command, add a command, scaffold a command. Use only when the user explicitly asks for it."
metadata:
  category: meta
---
...
**Hard rule:** You MUST use the ask the user for every question in this flow. ONE question per call. Wait for the answer before the next question. Do not batch.
...
2. Present it in ONE ask the user call as a single confirm-or-adjust gate: option A "Create it as specced" (show the name + one-line behaviour), option B "Let me adjust" (falls back to the full interview). Include the proposed name in the option label so the user sees it.
...
Ask ONE question via the ask the user:
...
Use the ask the user (single-select) with 3 options plus "Other" implicit:
...
Ask via the ask the user (single-select, 4 options):
...
Ask via the ask the user (free text, default to your suggested set):
...
Ask via the ask the user (free text):
...
Ask via the ask the user (single-select):
...
Ask via the ask the user (multi-select):
...
Write the file to `commands/<name>.md` using the write files.
