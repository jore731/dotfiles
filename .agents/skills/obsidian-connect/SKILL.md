...
description: "Bridge two unrelated domains using your vault's link graph - forces creative friction to spark new ideas Triggers: connect domains, cross-pollinate, bridge ideas, find an unexpected link. Use only when the user explicitly asks for it."
metadata:
  category: thinking
---
...
7. Offer to save the best connections to the ideas folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md` - wiki-style `wiki/concepts/`, Obsidian-style `Ideas/`) with links to both source domains
