...
description: "Find and resolve contradictions in the vault - the vault maintains its own truth Triggers: find contradictions, reconcile vault, fix conflicts, vault contradictions. Use only when the user explicitly asks for it."
metadata:
  category: thinking
---
...
3. Spawn parallel subagents to find contradictions:
   - **Claims agent**: scan the concepts and projects folders (resolved per `.agents/skills/obsidian-core/references/folder-map.md`) for factual claims - find pairs that contradict each other
   - **Entity agent**: scan the entities folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md`) for outdated roles, companies, or descriptions that conflict with newer sources
   - **Decisions agent**: scan the decisions folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md`) and project Key Decisions for reversed or superseded decisions that were never updated
...
5. Resolve each contradiction:
   ...
   - **Genuinely ambiguous**: create `Conflict - Topic.md` in the decisions folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md`; `type: conflict`, schema in `.agents/skills/obsidian-core/references/ai-first-rules.md`) documenting both sides, the evidence for each, and mark as `status: open` for the user to decide
