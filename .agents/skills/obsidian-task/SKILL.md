---
name: obsidian-task
description: "Add a task to the right kanban board with inferred priority and due date Triggers: add task, new todo, track this, remind me. Use proactively: trigger this whenever the conversation produces something worth capturing, without waiting to be asked."
metadata:
  category: vault
---
...
4. Search for the right kanban board - use `_CLAUDE.md` board list or search the boards folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md`)
5. Add the task card to the correct column (`📋 This Week` or `📥 Backlog` depending on due date)
6. Create a task note in the tasks folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md` - wiki-style `wiki/tasks/`, Obsidian-style `Tasks/`) if the task is substantial (more than a one-liner)
