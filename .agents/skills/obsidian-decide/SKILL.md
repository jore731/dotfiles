...
description: "Record decisions - lightweight by default (logged to project notes), or a full ADR record with --formal Triggers: extract decisions, log decisions, what did we decide, log this decision, ADR, record decision, decision record. Use proactively: trigger this whenever the conversation produces something worth capturing, without waiting to be asked."
metadata:
  category: thinking
---
...
2. Identify the structural decision - from the argument or recent conversation (a project graduated, a folder reorganized, a convention adopted, a concept promoted to hub). To surface decisions already made in code but never recorded, run `uv run --directory ".agents/skills/obsidian-core" scripts/mine_commit_decisions.py --repo <project> --json` (the skill root was given at session start as **Skill root**; substitute it for `.agents/skills/obsidian-core`) - it scans git history for decision-shaped commits ("switch to", "replace", "adopt", "rename", "migrate") and returns ADR candidates.
3. Create a decision record in the decisions folder resolved per `.agents/skills/obsidian-core/references/folder-map.md` (wiki-style `wiki/decisions/YYYY-MM-DD - Title.md`, Obsidian-style `Knowledge/ADR-YYYY-MM-DD - Title.md`), with frontmatter `date`, `type: adr`, `status: accepted`, `tags: [adr, decision]`, `ai-first: true`. Structure:
