---
name: x-pulse
description: "Scan X for what's trending in a topic - themes, voices, hooks, and post ideas powered by Grok x_search Triggers: x pulse, what is trending on twitter, scan x for, twitter pulse. Use only when the user explicitly asks for it."
metadata:
  category: research
---
...
2. Run the script from the skill root (its absolute path was given at session start as **Skill root**; substitute it for `.agents/skills/obsidian-core`):
   ```bash
   uv run --directory ".agents/skills/obsidian-core" -m scripts.research.x_pulse "<topic>"
   ```
