---
name: obsidian-panel
description: "Convene a panel of distinct perspectives on a decision - one independent verdict per lens, then a synthesis. A multi-persona complement to /obsidian-challenge Triggers: convene a panel, advisor panel, get multiple perspectives on, panel review, what would the experts say about. Use only when the user explicitly asks for it."
metadata:
  category: thinking
---
...
5. Save to the concepts folder (resolved per `.agents/skills/obsidian-core/references/folder-map.md` - wiki-style `wiki/concepts/`, Obsidian-style `Ideas/`) as `YYYY-MM-DD - panel - <slug>.md` (`type: synthesis`, tagged `[synthesis, thinking, panel]`), linking any `[[Advisors/...]]` notes and `[[entities/projects]]` the decision touches. Cross-link from today's daily note.
