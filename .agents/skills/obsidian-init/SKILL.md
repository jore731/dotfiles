---
name: obsidian-init
description: "Scan your vault and generate a _CLAUDE.md operating manual, index.md catalog, and log.md pointer Triggers: init vault, bootstrap vault, setup vault, scan vault. Use only when the user explicitly asks for it."
metadata:
  category: meta
---
...
4. Generate a complete `_CLAUDE.md` using the template bundled with the skill at `.agents/skills/obsidian-core/references/claude-md-template.md` (its absolute path was given at session start as **Skill root**), filled with real values from the vault
...
7. Create `Bases/` at the vault root if it does not exist. Stamp the four premade base files from `.agents/skills/obsidian-core/references/bases/` (the skill root given at session start):
...
If a monolithic `log.md` already exists with `## YYYY-MM-DD` sections: run `uv run --directory ".agents/skills/obsidian-core" scripts/migrate_log.py --vault <vault-path>` (its absolute path was given at session start as **Skill root**; substitute it for `.agents/skills/obsidian-core`) to split it into `Logs/YYYY-MM-DD.md` files. Do not overwrite manually.
